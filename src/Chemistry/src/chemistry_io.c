...
chemistry_t *read_chemistry_data(cstring_t chemistry_file)
{
    ...
    /* the elements */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_elements", HDF5Int, &n_elements);
        ...
        BM_GET_HDF5_DATASET_N(group_id, "symbol", HDF5String,
                              dims[0], elements->symbol);

        dims[0] = elements->n_elements;
        BM_GET_HDF5_DATASET_N(group_id, "mass", HDF5Double,
    ...
    /* the specii */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "n_specii", HDF5Int, &n_specii);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_reac_points", HDF5Int,
                              &max_reac_points);
        ...
        dims[0] = specii->n_specii;
        BM_GET_HDF5_DATASET_N(group_id, "is_inert", HDF5Int,
                              dims[0], specii->is_inert);

        dims[0] = specii->n_specii;
        dims[1] = chemistry->elements->n_elements;
        BM_GET_HDF5_DATASET_N_M(group_id, "composition", HDF5Int,
                                dims, specii->composition);

        dims[0] = specii->n_specii;
        BM_GET_HDF5_DATASET_N(group_id, "phase", HDF5Int,
                              dims[0], specii->phase);

        dims[0] = specii->n_specii;
        dims[1] = BOUNDS;
        BM_GET_HDF5_DATASET_N_M(group_id, "bounds", HDF5Double,
                                dims, specii->bounds);

        dims[0] = specii->n_specii;
        dims[1] = NASA;
        BM_GET_HDF5_DATASET_N_M(group_id, "coeff_high", HDF5Double,
                                dims, specii->coeff_high);

        dims[0] = specii->n_specii;
        dims[1] = NASA;
        BM_GET_HDF5_DATASET_N_M(group_id, "coeff_low", HDF5Double,
        ...
        dims[0] = specii->n_specii;
        BM_GET_HDF5_DATASET_N(group_id, "col_lj", HDF5Double,
                              dims[0], specii->col_lj);

        dims[0] = specii->n_specii;
        BM_GET_HDF5_DATASET_N(group_id, "dip_mo", HDF5Double,
        ...
        dims[0] = specii->n_specii;
        BM_GET_HDF5_DATASET_N(group_id, "n_reac_points", HDF5Int,
                              dims[0], specii->n_reac_points);

        dims[0] = specii->n_specii;
        dims[1] = specii->max_reac_points;
        BM_GET_HDF5_DATASET_N_M(group_id, "reac_points", HDF5Int,
                                dims, specii->reac_points);

        dims[0] = specii->n_specii;
        dims[1] = specii->max_reac_points;
        BM_GET_HDF5_DATASET_N_M(group_id, "nu_reac_points", HDF5Double,
    ...
    /* the reactions */
    {
        ...
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_troe_coeff", HDF5Int,
                              &max_troe_coeff);
        BM_GET_HDF5_ATTRIBUTE(group_id, "max_efficiencies", HDF5Int,
                              &max_efficiencies);
        ...
        dims[0] = reactions->n_reactions;
        dims[1] = ARR;
        BM_GET_HDF5_DATASET_N_M(group_id, "arr_coeff", HDF5Double,
                                dims, reactions->arr_coeff);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "is_reversible", HDF5Int,
                              dims[0], reactions->is_reversible);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "n_reactants", HDF5Int,
                              dims[0], reactions->n_reactants);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_reactants;
        BM_GET_HDF5_DATASET_N_M(group_id, "reactants", HDF5Int,
                                dims, reactions->reactants);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_reactants;
        BM_GET_HDF5_DATASET_N_M(group_id, "nu_reactants", HDF5Double,
                                dims, reactions->nu_reactants);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_reactants;
        BM_GET_HDF5_DATASET_N_M(group_id, "ord_reactants", HDF5Double,
                                dims, reactions->ord_reactants);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "n_products", HDF5Int,
                              dims[0], reactions->n_products);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_products;
        BM_GET_HDF5_DATASET_N_M(group_id, "products", HDF5Int,
                                dims, reactions->products);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_reactants;
        BM_GET_HDF5_DATASET_N_M(group_id, "nu_products", HDF5Double,
                                dims, reactions->nu_products);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_reactants;
        BM_GET_HDF5_DATASET_N_M(group_id, "ord_products", HDF5Double,
                                dims, reactions->ord_products);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "sum_nu", HDF5Double,
                              dims[0], reactions->sum_nu);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "has_rev_arr", HDF5Int,
                              dims[0], reactions->has_rev_arr);

        dims[0] = reactions->n_reactions;
        dims[1] = ARR;
        BM_GET_HDF5_DATASET_N_M(group_id, "rev_arr_coeff", HDF5Double,
                                dims, reactions->rev_arr_coeff);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "has_high_arr", HDF5Int,
                              dims[0], reactions->has_high_arr);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "has_low_arr", HDF5Int,
                              dims[0], reactions->has_low_arr);

        dims[0] = reactions->n_reactions;
        dims[1] = ARR;
        BM_GET_HDF5_DATASET_N_M(group_id, "adv_arr_coeff", HDF5Double,
                                dims, reactions->adv_arr_coeff);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "has_troe", HDF5Int,
                              dims[0], reactions->has_troe);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "n_troe_coeff", HDF5Int,
                              dims[0], reactions->n_troe_coeff);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_troe_coeff;
        BM_GET_HDF5_DATASET_N_M(group_id, "troe_coeff", HDF5Double,
                                dims, reactions->troe_coeff);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "has_efficiencies", HDF5Int,
                              dims[0], reactions->has_efficiencies);

        dims[0] = reactions->n_reactions;
        BM_GET_HDF5_DATASET_N(group_id, "n_efficiencies", HDF5Int,
                              dims[0], reactions->n_efficiencies);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_efficiencies;
        BM_GET_HDF5_DATASET_N_M(group_id, "sp_efficiencies", HDF5Int,
                                dims, reactions->sp_efficiencies);

        dims[0] = reactions->n_reactions;
        dims[1] = reactions->max_efficiencies;
        BM_GET_HDF5_DATASET_N_M(group_id, "efficiencies", HDF5Double,
