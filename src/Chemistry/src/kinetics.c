...
/*******************************************************************************
 * @brief Calculate the reaction rates
 * @param T
 * @param chemistry
 ******************************************************************************/
void calc_reaction_rates(double T, chemistry_t *chemistry)
{
    ...
    for (int ii = 0; ii < n_reactions_rev; ++ii)
    {
        ...
        tmp = exp(tmp);
        reactions->k[i * KINDIM + 1] =
            reactions->k[i * KINDIM] /
            (tmp * pow(s_prmt, reactions->sum_nu[i]));
    }
    ...
    for (int ii = 0; ii < n_reactions_rev_arr; ++ii)
    {
        ...
        reactions->k[i * KINDIM + 1] =
            calc_arr(T, &reactions->rev_arr_coeff[i * ARR]);
    }
}

/*******************************************************************************
 * @brief Calculate the rate of progress
 * @param C
 * @param T
 * @param chemistry
 ******************************************************************************/
void calc_rate_of_progress(double *C, double T, chemistry_t *chemistry)
{
    ...
    for (int i = 0; i < n_reactions; ++i)
    {
        ...
        for (int j = 0; j < reactions->n_reactants[i]; ++j)
        {
            ...
            if (use_sp[reactions->reactants[idx_sp]] == 0)
                is_zero = 1;
            tmp += log(C[reactions->reactants[idx_sp]]) *
                   reactions->ord_reactants[idx_sp];
        }

        reactions->q[i * KINDIM] =
            (is_zero == 0) ? reactions->k[i * KINDIM] * exp(tmp) : 0.0;
    }
    ...
    for (int ii = 0; ii < n_reactions_rev; ++ii)
    {
        ...
        for (int j = 0; j < reactions->n_products[i]; ++j)
        {
            ...
            if (use_sp[reactions->products[idx_sp]] == 0)
                is_zero = 1;
            tmp += log(C[reactions->products[idx_sp]]) *
                   reactions->ord_products[idx_sp];
        }

        reactions->q[i * KINDIM + 1] =
            (is_zero == 0) ? reactions->k[i * KINDIM + 1] * exp(tmp) : 0.0;
    }
    ...
    for (int ii = 0; ii < n_reactions_three; ++ii)
    {
        ...
        for (int j = 0; j < reactions->n_efficiencies[i]; ++j)
        {
            ...
            C[n_specii] += (reactions->efficiencies[idx_sp] - 1.0) *
    ...
    for (int ii = 0; ii < n_reactions_troe; ++ii)
    {
        ...
        double tmp = calc_troe(T, reactions->pr[i],
                               &reactions->troe_coeff[i * max_troe_coeff]);
...
void calc_production_rate(double *C, double T, chemistry_t *chemistry)
{
    ...
    for (int i = 0; i < n_specii; ++i)
    {
        ...
        for (int j = 0; j < specii->n_reac_points[i]; ++j)
        {
            ...
            specii->omega[i] +=
                specii->nu_reac_points[i * max_reac_points + j] *
...
double calc_troe(double T, double pr, double *coeff)
{
    ...
    double fcn_tr = (1. - coeff[0]) * exp(-T / coeff[1]) +
                    coeff[0] * exp(-T / coeff[2]) + exp(-coeff[3] / T);
