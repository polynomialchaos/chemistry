...
class Reaction(Base):
    ...
    def is_elementar(self):
        digit_nu_reactants = all(
            self.reactants[key].is_integer() for key in self.reactants)
        digit_nu_products = all(
            self.products[key].is_integer() for key in self.products)
    ...
    def product_orders(self):
        tmp = [key for key in self.products]
        return [self.r_orders.get(key, self.products[key]) for key in tmp]
    ...
    def reactant_orders(self):
        tmp = [key for key in self.reactants]
        return [self.f_orders.get(key, self.reactants[key]) for key in tmp]
...
def check_element_balance(elements, species, reaction, limit=1e-5):
    ...
    for x, y in reaction.reactants.items():
        for key in species[x].thermo.composition:
