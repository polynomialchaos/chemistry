...
class Mechanism( Base ):
    ...
    def chemkinify( self, prefix, unit_k0=None, unit_Ea=None ):
        with open( '{:}.mech'.format( prefix ), 'w' ) as fp:
            ...
            if unit_k0 is None: unit_k0 = self.reactions.unit_k0
            if unit_Ea is None: unit_Ea = self.reactions.unit_Ea

            tmp = 'REACTIONS'
            if unit_k0 != def_unit_k0: tmp += ' {:}'.format( unit_k0 )
            if unit_Ea != def_unit_Ea: tmp += ' {:}'.format( unit_Ea )

            fp.writelines( ['{:}\n'.format( tmp )] + self.reactions.chemkinify( unit_k0=unit_k0, unit_Ea=unit_Ea )+ ['END\n'] )
        ...
        with open( '{:}.transport'.format( prefix ), 'w' ) as fp:
            fp.writelines( self.transports.chemkinify( **{'keys': self.specii.keys()} ) )
