...
def as_int(value, round_digits=None):
    ...
    tmpValue = float(value) if round_digits is None else round(
        value, round_digits)
...
def f_arr_ta(T, coeff):
    """Return the arrhenius function (using act. temperature)."""
    return f_arr(T, coeff, conv=1.0)


def f_arr_ea(T, coeff):
    """Return the arrhenius function (using act. energy)."""
...
def alpha_to_arr(c_alpha, T_alpha_max, n_alpha):
    ...
    return [c_alpha * T_alpha_max**n_alpha * np.exp(n_alpha),
            -n_alpha, n_alpha * T_alpha_max]
