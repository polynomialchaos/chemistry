...
class Species( Base ):
    ...
    def molar_mass( self ):
        if hasattr( self, '_molar_mass' ):
            ...
            raise( ValueError( 'Neither molar_mass nor elements/thermo properties ' \
    ...
    def _checklist( self ):
        ...
        if self.thermo:
            ...
            cp_poly         = self.get_dimless_cp( T )

            add_checks.append(
                (all( cp_poly >= 0.0 ), 'Cp polynomial not valid (reduce bounds to {:.2f})'.format( T[np.where( cp_poly >= 0 )[-1][-1]] )),
            )

        return [
            (self.thermo is not None, 'Missing thermo data'),
            (self.transport is not None, 'Missing transport data'),
            (self.molar_mass >= 0.0, 'Molar mass not valid (mm={:})'.format( self.molar_mass )),
        ] + add_checks

    def get_dimless_cp( self, T ):
        return self._numpy_handler( T, calc_dimless_cp )

    def get_dimless_g( self, T ):
        return self._numpy_handler( T, calc_dimless_g )

    def get_dimless_h( self, T ):
        return self._numpy_handler( T, calc_dimless_h )

    def get_dimless_s( self, T ):
        return self._numpy_handler( T, calc_dimless_s )

    def _numpy_handler( self, T, function ):
        if isinstance( T, (list, np.ndarray) ):
            T_l, T_h = split_at( T, self.thermo.bounds[1] )
