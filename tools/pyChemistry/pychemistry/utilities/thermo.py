...
class Thermo( Base ):
    ...
    @property
    def coeff_high( self ):
        return self._coeffHigh

    @property
    def coeff_low( self ):
        return self._coeffLow
    ...
    def coeff_high( self, value ):
        self._coeffHigh = [float( x ) for x in value]

    @coeff_low.setter
    def coeff_low( self, value ):
        self._coeffLow = [float( x ) for x in value]
    ...
    def chemkinify( self ):
        ...
        return '{:79}1{:}\n{:79}2\n{:79}3\n{:79}4'.format(
            ...
            ''.join( ['{: 15.8E}'.format( coeff ) for coeff in self.coeff_high[:5]] ),
            ''.join( ['{: 15.8E}'.format( coeff ) for coeff in self.coeff_high[5:] + self.coeff_low[:3]] ),
            ''.join( ['{: 15.8E}'.format( coeff ) for coeff in self.coeff_low[3:]] ),
        )

    def _checklist( self ):
        return [
            (bool( self.composition ), 'Elemental composition empty'),
            (len( self.bounds ) == N_BOUNDS, 'NASA temp. bounds of wrong size ({:})'.format( len( self.bounds ) )),
            (len( self.coeff_low ) == N_NASA, 'NASA low temp. coefficients of wrong size ({:})'.format( len( self.coeff_low ) )),
            (len( self.coeff_high ) == N_NASA, 'NASA high temp. coefficients of wrong size ({:})'.format( len( self.coeff_high ) )),
            (self.phase == 0, 'Phase not valid'),
        ]

    def _get_comp_strings( self ):
        all_comps = ['{:2}{:3}'.format( key, int( self.composition[key] ) ) for key in self.composition]
