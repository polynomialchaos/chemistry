...
def chemkin_format_reader( path, start_keys=None, end_keys=None, comment='!' ):
    ...
    start_keys = sorted( [] if start_keys is None else start_keys, key=(lambda x: len( x )), reverse=True )
    end_keys = sorted( [] if end_keys is None else end_keys, key=(lambda x: len( x )), reverse=True )
    ...
    for _, line in enumerate( strings ):
        ...
        if line.strip()[0] == comment: continue

        # start the append mode if a startKey is present in the current line
        if any( key in line for key in start_keys ): app_mode = True
        ...
        if app_mode and line.rstrip():
            ...
            parsed_data.append( (tmpLine[0].rstrip(), tmpLine[1].rstrip() if tmpLine[1:] else '') )

        # end append mode if no endKey is in strings or
        # if a endKey is present in the current line
        if app_mode and any( key in line for key in end_keys): break

    if parsed_data:
        # look for the keywords in the first line
        for key in start_keys:
            if key in parsed_data[0][0]:
                parsed_data[0] = (parsed_data[0][0].replace( key, '' ), parsed_data[0][1])

        # look for the keywords in the last line
        for key in end_keys:
            if key in parsed_data[-1][0]:
                parsed_data[-1] = (parsed_data[-1][0].replace( key, '' ), parsed_data[0][1])
