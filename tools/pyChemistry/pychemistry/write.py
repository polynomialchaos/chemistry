...
def write_chemistry_data(path, mechanism):
    ...
    getElCount = (
        lambda x: [x[symbol] if symbol in x else 0.0 for symbol in elSymbol])
    ...
    spReacPoints, spReacNus = get_specii_reaction_points(
        spSymbol, mechanism.reactions)
    ...
    reReactants = [[spSymbolM.index(y) for y in x.reactants]
                   for x in mechanism.reactions]
    reNuReactants = [[x.reactants[y] for y in x.reactants]
                     for x in mechanism.reactions]
    reOrdReactants = [x.reactant_orders for x in mechanism.reactions]

    reNProducts = [len(x.products) for x in mechanism.reactions]
    reProducts = [[spSymbolM.index(y) for y in x.products]
                  for x in mechanism.reactions]
    reNuProducts = [[x.products[y] for y in x.products]
                    for x in mechanism.reactions]
    ...
    reHasRevArr = [
        True if x.rev_arr_coeff else False for x in mechanism.reactions]
    reRevArrCoeff = [x.rev_arr_coeff for x in mechanism.reactions]

    reHasHighArr = [True if x.adv_arr_key ==
                    'HIGH' else False for x in mechanism.reactions]
    reHasLowArr = [True if x.adv_arr_key ==
                   'LOW' else False for x in mechanism.reactions]
    ...
    with h5py.File(path, 'w') as h5f:
        ...
        group.create_dataset(
            r'nu_reactants', data=fillArray(reNuReactants, 0.0))
        group.create_dataset(
            r'ord_reactants', data=fillArray(reOrdReactants, 0.0))
        ...
        group.create_dataset(
            r'ord_products', data=fillArray(reOrdProducts, 0.0))

        group.create_dataset(r'sum_nu', data=reSumNu)

        group.create_dataset(r'has_rev_arr', data=reHasRevArr)
        group.create_dataset(r'rev_arr_coeff', data=fillArray(
            reRevArrCoeff, 0.0, maxLength=(lambda x: 3)))
        group.create_dataset(r'has_high_arr', data=reHasHighArr)
        group.create_dataset(r'has_low_arr', data=reHasLowArr)
        group.create_dataset(r'adv_arr_coeff', data=fillArray(
            reAdvArrCoeff, 0.0, maxLength=(lambda x: 3)))
        group.create_dataset(r'has_troe', data=reHasTroe)
        group.create_dataset(r'n_troe_coeff', data=reNTroeCoeff)
        group.create_dataset(r'troe_coeff', data=fillArray(
            reTroeCoeff, 0.0, maxLength=(lambda x: 4)))

        group.create_dataset(r'has_efficiencies', data=reHasEfficiencies)
        group.create_dataset(r'n_efficiencies', data=reNEfficiencies)
        group.create_dataset(r'sp_efficiencies',
                             data=fillArray(reSpEfficiencies, -1))
        group.create_dataset(
            r'efficiencies', data=fillArray(reEfficiencies, 0.0))
